 ...
 */
define(['createjs', 'jumpy/core/GameConfig'], function (createjs, GameConfig) {
  // ===========================================
  //  Private Constants
  // ===========================================
  /**
  ...
  function ConnectionManager() {
    if (instance !== null) {
      throw new Error(
        'Cannot instantiate more than one ConnectionManager, use ConnectionManager.getInstance()'
      );
    }
  ...
  ConnectionManager.prototype.connect = function () {
    ...
    this._peer.on(
      ...
      function (error) {
        ...
        this.dispatchEvent(
          new createjs.Event(ConnectionManager.CONNECTION_ERROR)
        );
      }.bind(this)
    );
  };
  ...
  ConnectionManager.prototype._addPeer = function (conn) {
    ...
    if (
      this._peers.filter(function (peerConn) {
        return peerConn.peer === conn.peer;
      }).length === 0
    ) {
  ...
  ConnectionManager.prototype._connectToAdmin = function () {
    ...
    adminConn.on(
      ...
      function () {
        ...
        this.dispatchEvent(
          new createjs.Event(ConnectionManager.CONNECTION_SUCCESS)
        );
      }.bind(this)
    );
    adminConn.on(
      ...
      function (data) {
        ...
        switch (data.action) {
          case 'wait':
            this.dispatchEvent(
              new createjs.Event(ConnectionManager.WAIT_FOR_OTHERS)
            );
            break;

          case 'startTime':
            event = new createjs.Event(
              ConnectionManager.GAME_START_TIME_RECEIVED
            );
          ...
          case 'start':
            ...
            event = new createjs.Event(
              ConnectionManager.GAME_END_TIME_RECEIVED
            );
