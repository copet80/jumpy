  ...
  $(document).ready(function () {
    ...
    function addResult(winners) {
      var createPeerColumn = function (peerConn) {
        ...
        if (peerConn) {
          ...
          animal.css(
            'background-position',
            '0 ' + ANIMALS.indexOf(peerConn.animalId) * -44 + 'px'
          );
        ...
        Object.keys(peersByGameId).forEach(function (gameId) {
          ...
          if (!table) {
            table = $(
              '<table id="' +
                gameId +
                '" class="table table-condensed table-bordered game">' +
                '<thead>' +
                '<tr>' +
                '<th colspan="5">' +
                'Game #<span class="game-id">' +
                gameId +
                '</span>' +
                '<br>' +
          ...
          if (peers.length === 0) {
            ...
            if (peers.length === 0) {
              table
                .find('tbody')
                .html('<tr><td colspan="4">No players yet.</td></tr>');
            } else {
    ...
    function resetCountdown() {
      if (
        ...
        getPeersByStatus(STATUS_JOINING).length <= 2
      ) {
    ...
    function updateCountDown() {
      ...
      if (startingPeers.length >= 2) {
        ...
        var secs = Math.floor(
          (COUNTDOWN_TIME - (now - countdownStartTime)) * 0.001
        );
        $('#countdown').text(secs);
        ...
        if (currentCountDownSeconds !== secs) {
          ...
          if (secs <= 0) {
            ...
            removePeersFromQueueById(
              peersClone.map(function (peer) {
                return peer.peer;
              })
            );
            peersClone.forEach(function (peer) {
              ...
              peer.send({
                ...
                peerIds: peersClone.map(function (peer) {
                  return peer.peer;
                }),
              });
            });
